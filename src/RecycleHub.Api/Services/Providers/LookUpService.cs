    ...
    IUnitOfWork unitOfWork) : ILookUpService
{
    ...
    public async Task<ApiResponse<bool>> DeleteMaterialAsync(string id, CancellationToken ct = default)
    {
        try
        {
            var material = await unitOfWork.Materials.GetByIdAsync(id, ct);
            if (material == null)
                return ApiResponse<bool>.Fail("Material not found", StatusCodes.Status404NotFound);

            material.IsDeleted = true;
            material.UpdatedAt = DateTime.UtcNow;
            await unitOfWork.Materials.UpdateAsync(material, saveChanges: true, ct: ct);

            return true.ToApiResponse("Material deleted successfully", StatusCodes.Status200OK);
        }
        catch (Exception e)
        {
            logger.LogError(e, "Error deleting material {Id}: {Message}", id, e.Message);
            return ApiResponse<bool>.Fail();
        }
    }
}
